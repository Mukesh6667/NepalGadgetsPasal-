...
function renderProducts(filter = "all") {
  ...
  if (!list.length) {
    grid.innerHTML = `<div style="grid-column:1/-1;text-align:center;padding:60px;color:var(--muted)"><i class="fa fa-box-open" style="font-size:40px;opacity:0.15;display:block;margin-bottom:12px"></i>No products in this category.</div>`;
    return;
  }
  grid.innerHTML = list.map((p, i) => {
    ...
    return `<div class="pcard" style="animation:cardin 0.4s ease ${i * 0.04}s both">
      <div class="pimg">
        ...
        <button class="wish-btn ${iw ? "on" : ""}" data-id="${p.id}" title="Wishlist">
          <i class="${iw ? "fas" : "far"} fa-heart"></i>
        </button>
        <img src="${p.img}" alt="${p.name}" loading="lazy"
          onerror="this.style.display='none';this.nextElementSibling.style.display='flex'"/>
        <i class="fas ${p.icon} fallback-icon"></i>
        <button class="qview-btn" data-id="${p.id}"><i class="fa fa-eye"></i> Quick View</button>
      </div>
      <div class="pinfo">
        <div class="pbrand">${p.brand}</div>
        <div class="pname" data-id="${p.id}">${p.name}</div>
        ...
        <button class="btn-primary add-cart-btn" data-id="${p.id}">
  ...
  grid.querySelectorAll(".qview-btn, .pname").forEach(el =>
...
// ================================================================
// FIREBASE — LOAD PRODUCTS (real-time)
// ================================================================
async function loadProductsFromFirebase() {
  try {
    ...
    console.log("Using local products:", e.message);
  }
  renderProducts();
}
